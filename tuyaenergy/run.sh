...
echo "Parameters: $MQTTSERVER $MQTTUSER $MQTTPASSWORD $MQTTTOPIC $MQTTPORT $TUYADEVIPS $TUYADEVIDS $TUYADEVKEYS $TUYADEVVERS"

while true; do
  python tuyaConsuption.py $MQTTSERVER $MQTTUSER $MQTTPASSWORD $MQTTTOPIC $MQTTPORT $TUYADEVIPS $TUYADEVIDS $TUYADEVKEYS $TUYADEVVERS || true
  sleep 60
done
